...
INSERT INTO gads_dim_next.ad_attribute_value
  ...
  WITH attributes AS (
      SELECT DISTINCT (jsonb_each_text(_attributes)).*
...
CREATE FUNCTION gads_tmp.constrain_ad_attribute_mapping()
  RETURNS VOID AS $$
SELECT util.add_fk('gads_dim_next', 'ad_attribute_mapping', 'gads_dim_next', 'ad');
SELECT util.add_fk('gads_dim_next', 'ad_attribute_mapping', 'gads_dim_next', 'ad_attribute_value');
SELECT util.add_index('gads_dim_next', 'ad_attribute_mapping',
